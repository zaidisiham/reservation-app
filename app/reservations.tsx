...
export default function MesReservations() {
  ...
  useEffect(() => {
    fetch(`http://192.168.1.69/reservation-app/api/get_mes_reservations.php?utilisateur_id=${utilisateurId}`)
      .then(res => res.json())
    ...
    Alert.alert('Confirmer', 'Supprimer cette réservation ?', [
      { text: 'Annuler' },
      {
        text: 'Supprimer', onPress: async () => {
          try {
            const res = await fetch(`http://192.168.1.69/reservation-app/api/delete_reservation.php?id=${id}`, {
              method: 'DELETE'
            });
