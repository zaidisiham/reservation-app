...
export default function ReserverPage() {
  ...
  const checkDisponibilite = async () => {
    ...
    try {
      const res = await fetch(
        `http://192.168.1.69/reservation-app/api/check_disponibilite.php?equipement_id=${id}&date_debut=${dateDebut}&date_fin=${dateFin}`
      );
  ...
  const confirmerReservation = async () => {
    ...
    try {
      const res = await fetch('http://192.168.1.69/reservation-app/api/reserver_equipement.php', {
        method: 'POST',
